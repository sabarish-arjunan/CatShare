...
export async function saveRenderedImage(product, type, units = {}) {
  ...
  if (product.customFields && product.customFields.length > 0) {
    ...
    attributesHtml = product.customFields.map((field) => {
      ...
      return fieldValue ? `<div style="display:flex;margin:2px 0;padding-left:8px"><span style="min-width:90px">${field.name}</span><span style="margin-right:12px">:</span><span>${fieldValue}</span>${showUnits && fieldUnit !== "N/A" ? `<span style="margin-left:4px">${fieldUnit}</span>` : ""}</div>` : "";
    }).join("");
  } else {
    ...
    attributesHtml = `
      ${showColour ? `<div style="display:flex;margin:2px 0;padding-left:8px"><span style="min-width:90px">Colour</span><span style="margin-right:4px">:</span><span>${product.color}</span></div>` : ""}
      ${showPackage ? `<div style="display:flex;margin:2px 0;padding-left:8px"><span style="min-width:90px">Package</span><span style="margin-right:4px">:</span><span>${product.package} ${displayPackageUnit}</span></div>` : ""}
      ${showAgeGroup ? `<div style="display:flex;margin:2px 0;padding-left:8px"><span style="min-width:90px">Age Group</span><span style="margin-right:4px">:</span><span>${product.age} ${displayAgeUnit}</span></div>` : ""}
    `;
  }
