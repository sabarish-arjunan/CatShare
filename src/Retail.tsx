                ...
                <button onClick={async () => {
                  try {
                    ...
                    if (imagePreview && imagePreview.startsWith('data:image')) {
                      ...
                      // Also save a copy to documents folder for sharing (consistent filename pattern)
                      try {
                        const shareFilename = `product_${id}_retail.png`;
                        await Filesystem.writeFile({ path: `Retail/${shareFilename}`, data: base64, directory: Directory.Documents, recursive: true });
                      } catch (err) {
